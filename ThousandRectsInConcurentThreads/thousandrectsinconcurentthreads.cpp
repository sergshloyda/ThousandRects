	...
	: QWidget(parent),_result_data(1000)
{
	...
	//включим дл€ 1 канала экстра порог 2 строба
...
void ThousandRectsInConcurentThreads::received_data()
{
	...
	const par_thick_t &curr_thick_par = _pDeviceSettings->getThickParams();//задаетс€ в диалоге настроек
	...
	for(int i=0;i<info_header.count;i++)
	{
		...
		for(quint8 ii=0;ii<_pDeviceSettings->getChansCount();ii++)
		{
			...
			{
				//дл€ B скана установим порог дл€ тестировани€
